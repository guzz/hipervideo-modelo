...
<template>
  <div id="redes" v-show="glossario" :class="{'is-redes': glossario}" transition="rede">
    <div v-el:redesgraph class="redes_graph redes_graph_svg">
      <div class="legendas">
        <div @click="filter('mulher')" class="leg" :class="{'is-ativo': filter_mulher}" style="padding: 3% 2% 1%;">MULHER</div>
        <div @click="filter('crianca')" class="leg" :class="{'is-ativo': filter_crianca}" style="padding: 3% 2% 1%;">CRIANÇA</div>
        <div @click="filter('adolescente')" class="leg" :class="{'is-ativo': filter_adolescente}" style="padding: 3% 2% 1%;">ADOLESCENTE</div>
        <div @click="filter('deficiencia')" class="leg" :class="{'is-ativo': filter_deficiencia}">PESSOA COM DEFICIENCIA</div>
        <div @click="filter('prisional')" class="leg" :class="{'is-ativo': filter_prisional}">PESSOA PRIVADA DE LIBERDADE</div>
      </div>
    </div>
    <div class="redes_info">
      <in-redes-info :glossario.sync="glossario"></in-redes-info>
    </div>
  </div>
</template>
  ...
  module.exports = {
    ...
    methods: {
      ...
      createGraph: function(){
        ...
        this._svg = d3.select(this.$els.redesgraph).append("svg")
          .attr("width", radius)
          .attr("height", radius);
      ...
      updateGraph: function(){
        ...
        var node = this._svg_nodes.selectAll(".node")
          ...
          .on('click',function(d){
            ...
            self.$broadcast('create-conteudo', d)
          })
