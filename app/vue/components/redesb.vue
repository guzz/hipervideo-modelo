    ...
    methods: {
      ...
      addRootNode: function(){
        for (var i = 0; i < this.database.length; i++) {
          var conect = _.findWhere(this.database[i].plugins, {tipo: "conexoes"})
          if (conect !== undefined) {
            var c = this.database[i].eventos
            var e = []
            var n
            if (this.hipergroups.length > 0) {
              var offset = 0
              for (var a = 0; a < this.hipergroups.length; a++) {
                offset = this.hipergroups[a].count + offset
              };
              for (var o = 0; o < conect.edges.length; o++) {
                var ed = _.mapObject(conect.edges[o], function (val, key) {
                  return val + offset
                })
                e.push(ed)
              }
            ...
            n.eventos = c
            n.edges = e
            n.count = n.eventos.length

            this.hipergroups.push(n)
          }
        }
      },
      ...
      populateGraph: function(area){
        ...
        for (var i = 0; i < this.hipergroups.length; i++) {
          this.hipergroups[i]
        };
