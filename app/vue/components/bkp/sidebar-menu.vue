...
<template>
  <div class="sidebar__menu" :class="{'is-open': isOpen, 'is-after': isAfter, 'is-almost': isAlmost}">
    <div @click="toggle" style="position: absolute; left: 0px; top: 0px; color: #555; cursor: pointer;" class="menu_handle"><i class="fa fa-bars fa-2x" style="position: absolute; left: 10px; top: 6px;"></i></div>
    <div class="menu_1">
      <div class="menu_item" :class="{clicado: menuAcess}" @click="clickAcess">ACESSIBILIDADE</div> 
      <div class="menu_item" :class="{clicado: menuQual}" @click="clickQual">QUALIDADE</div>
      <div class="menu_item" :class="{clicado: menuHip}" @click="clickHip">HIPERVÍDEOS</div>
      <div class="menu_item" @click="clickRedes">VER REDES</div>
      <a href="/#/" class="menu_item">INÍCIO</a>
      <div class="menu_item" @click="clickCreditos">CRÉDITOS</div>
    </div>
      <div class="menu_2" v-show="menuAcess">
      <div class="menu_item" :class="{selecionado: isNada}" @click="selectNada">SEM ACESSIBILIDADE</div>
      <div class="menu_item" :class="{selecionado: isAudio}" @click="selectAudio">ÁUDIO DESCRIÇÃO</div>
      <div class="menu_item" :class="{selecionado: isLibras}" @click="selectLibras">LIBRAS</div>
    </div>
    <div class="menu_2" v-show="menuQual">
      <div class="menu_item" :class="{selecionado: isAlta}" @click="selectAlta">ALTA</div>
      <div class="menu_item" :class="{selecionado: isMedia}" @click="selectMedia">MÉDIA</div>
      <div class="menu_item" :class="{selecionado: isBaixa}" @click="selectBaixa">BAIXA</div>
    </div>
